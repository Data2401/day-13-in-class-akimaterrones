...
my_flights %>% 
  group_by(dest) %>%
  summarise(avg_delay = mean(arr_delay)) -> dest_delays
glimpse(dest_delays)
glimpse(airports)
```
...
```{r}
left_join(dest_delays, airports, by = join_by(dest == faa))
```
...
```{r}
left_join(dest_delays, airports, by = join_by(dest == faa)) %>% arrange(-avg_delay)
```
...
my_flights %>% 
  ...
  left_join(airlines) -> airline_delay
airline_delay
